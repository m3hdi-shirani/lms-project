...
const CardPlaceholder: React.FC<PlaceholderCardProps> = ({
  count = 1,
  className,
  ...
  return (
    <div
      className={`flex flex-wrap justify-center xl:justify-start gap-6 mt-10 h-96 ${className}`}
    >
      {counts.map((item) => (
        <div
          key={`cardplaceholder${item}`}
          role="status"
          className="w-80 min-h-96 flex-1 rounded animate-pulse"
        >
